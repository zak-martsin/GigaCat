...
The artwork action reads `Add` for an unsaved program and `Remove` for a program already in the
current user's Library. Removing a program requires confirmation.
